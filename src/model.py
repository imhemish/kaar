...
class TaskStack(Gtk.Stack):
    ...
    check_label = Gtk.Template.Child()
...
class TaskFactory(Gtk.SignalListItemFactory):
    ...
    def bind_task_item(self, fact, list_item):
        ...
        task_stack.entry_row.bind_property("text", task_object, "line")
        if task_object.completed:
            task_stack.check_label.set_visible(True)
        task_object.bind_property("completed", task_stack.check_label, "visible")
